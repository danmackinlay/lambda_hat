...
def test_sgld_noise_independence_smoke():
    ...
    def loss_full(p):
        return 0.5 * (jnp.sum(p["a"] ** 2) + jnp.sum(p["b"] ** 2)).astype(jnp.float32)
    ...
    tb = TargetBundle(
        ...
        params0_f32=params,
        params0_f64=None,
        loss_full_f32=loss_full,
        loss_minibatch_f32=loss_mb,
        loss_full_f64=None,
        loss_minibatch_f64=None,
        X=jnp.zeros((8, 1), jnp.float64),
        Y=jnp.zeros((8, 1), jnp.float64),
        L0=0.0,
        model=None,
    )
    cfg = type("C", (), {})()
