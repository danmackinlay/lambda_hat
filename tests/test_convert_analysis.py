...
def test_to_idata_and_metrics():
    ...
    H = [np.linspace(1.0, 0.8, 120), np.linspace(1.05, 0.85, 100), np.linspace(0.95, 0.82, 110)]
    idata = to_idata(
        Ln_histories=H, theta_thin=None, acceptance=None, energy=None,
        n=1000, beta=0.2, L0=0.7
    )
...
def test_to_idata_with_theta():
    ...
    theta = [np.random.randn(200, 10), np.random.randn(200, 10)]  # 2 chains, 200 draws, 10 dims
    idata = to_idata(
        Ln_histories=H, theta_thin=theta, acceptance=None, energy=None,
        n=500, beta=0.1, L0=0.0, max_theta_dims=5
    )
...
def test_to_idata_with_hmc_extras():
    ...
    idata = to_idata(
        Ln_histories=H, theta_thin=None, acceptance=acc, energy=energy,
        n=500, beta=0.1, L0=0.0
    )
...
def test_rank_plot():
    ...
    H = [np.random.randn(200)+1.0, np.random.randn(200)+1.0]
    idata = to_idata(Ln_histories=H, theta_thin=None, acceptance=None, energy=None, n=500, beta=0.1, L0=0.0)
...
def test_empty_histories():
    ...
    with pytest.raises(ValueError, match="Ln_histories is empty"):
        to_idata(
            Ln_histories=[], theta_thin=None, acceptance=None, energy=None,
            n=500, beta=0.1, L0=0.0
        )
...
def test_ragged_alignment():
    ...
    H = [np.random.randn(150)+1.0, np.random.randn(100)+1.0, np.random.randn(120)+1.0]
    # Different length acceptance (should be aligned to shortest)
    acc = [np.random.uniform(0.5, 1.0, 150), np.random.uniform(0.5, 1.0, 80), np.random.uniform(0.5, 1.0, 120)]

    idata = to_idata(
        Ln_histories=H, theta_thin=None, acceptance=acc, energy=None,
        n=500, beta=0.1, L0=0.0
    )
