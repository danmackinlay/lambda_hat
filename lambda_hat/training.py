...
def train_erm(
    ...
    # Training loop
    losses = []
    for step in range(cfg.erm_steps):
        params, opt_state, loss_val = update(params, opt_state)
        losses.append(float(loss_val))

        # Early stopping check
        if step > 100 and cfg.early_stop_tol is not None:
            recent_losses = losses[-20:]
            if len(recent_losses) == 20:
                std = jnp.std(jnp.array(recent_losses))
                if std < cfg.early_stop_tol:
                    print(f"Early stopping at step {step}, std={std:.2e}")
                    break

        # Logging
        if step % max(1, cfg.erm_steps // 10) == 0:
            print(f"Step {step}/{cfg.erm_steps}, Loss: {loss_val:.6f}")
