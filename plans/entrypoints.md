...
(This mirrors the `llc` setup, giving you a single `lambda-hat` entrypoint for everything.)
...
If you want, I can produce the exact `commands/*` modules by lifting code directly from the current entrypoints and wiring them to the new CLI—no compatibility shims, just the shortest path to a single clean interface.
