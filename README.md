# Lambda-Hat: Local Posterior (LLC) Sampler Benchmarks
...
2. Run a small experiment:
   ...
   ```bash
   uv run lambda-hat run --sampler sgld --preset=quick
   ```

3. Analyze the run:

   ```bash
   uv run lambda-hat analyze runs/<run_id> --which all --plots running_llc,rank,autocorr,energy,theta
   ```
...
More plots are available via `lambda-hat analyze` (rank, ESS, autocorrelation, theta traces, energy histograms).
...
```bash
uv sync --extra cpu
uv run lambda-hat sweep --study study_default.yaml --gpus 0,1,2  # GPU sweep
uv run lambda-hat sweep --study study_small.yaml               # CPU sweep
```
...
```bash
uv run lambda-hat run --sampler sgld --preset=quick
uv run lambda-hat run --sampler sgnht --preset=quick
uv run lambda-hat run --sampler hmc --preset=quick
uv run lambda-hat run --sampler mclmc --preset=quick
uv run lambda-hat run --sampler sgld --target=dln --preset=quick  # DLN target
```
...
| Task                    | Command                                                     |
| ----------------------- | ----------------------------------------------------------- |
| Single SGLD run         | `uv run lambda-hat run --sampler sgld --preset=quick`             |
| Single SGNHT run        | `uv run lambda-hat run --sampler sgnht --preset=quick`            |
| Single HMC run          | `uv run lambda-hat run --sampler hmc --preset=quick`              |
| Single MCLMC run        | `uv run lambda-hat run --sampler mclmc --preset=quick`            |
| Local GPU sweep         | `uv run lambda-hat sweep --study study_default.yaml --gpus 0,1`   |
| Local CPU sweep         | `uv run lambda-hat sweep --study study_small.yaml`                |
| Analyze saved run       | `uv run lambda-hat analyze runs/<run_id>`                         |
| Debug with verbose mode | `uv run lambda-hat --verbose run --sampler sgld`                  |
...
└── analysis/            # generated by `lambda-hat analyze`
    ├── *_running_llc.png
    ├── *_rank.png
    └── ...
```
...
```bash
uv run lambda-hat sweep --study study_small.yaml
```
...
```bash
uv run lambda-hat analyze runs/<run_id> --which all \
